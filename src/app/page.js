...
import { Button, Form, Textarea, MessagePlugin } from 'tdesign-react';
...
export default function Names(a, b) {
  ...
  return (
    <div>
      ...
      <main className='main' style={{ maxWidth: '800px', minWidth: '300px', width: '80%', margin: '0 auto', height: '100vh' }}>
        ...
        <Form colon={false} ref={formRef} labelAlign='right' labelWidth='100px' layout='vertical' preventSubmitDefault resetType='empty' showErrorMessage form={form} scrollToFirstError='smooth' submitWithWarningMessage={false} onSubmit={onSubmit} className='form'>
          <FormItem
            name='content'
            rules={[
              { required: true, message: '请输入要翻译的文字', type: 'error' },
              { max: 30000, message: '请最多输入30000个字符', type: 'error' },
            ]}
          >
          ...
          <Textarea allowInputOverMax={false} autofocus={false} autosize={{ minRows: 6 }} readonly value={messages} />
        </Form>
      </main>
    </div>
  );
